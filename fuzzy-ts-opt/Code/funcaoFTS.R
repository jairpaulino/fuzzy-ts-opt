getFTS= function(time.series, D1, D2, n, C){ 
  ...
  # time.series = dados; D1 = 1100; D2 = 1800; n = 7; C = 0.0001
...
getSerieDiff= function(time.series, D1, D2, n){ 
  ...
  return(list(uDiff, Vmin))
} 

getDiscourseUniverse= function(n, Vmin){ 
  ...
  return(discourseUniverse)
} 

getfuzzification = function(time.series, ts.diff, n, C, uim){
  
  ###Step_3.Definition of fuzzy sets in the universe of U speech and Fuzzification
  #time <- 1980:2001 #periodo
  time = min(time.series$time):max(time.series$time)
  
  Ai <- 1:length(ts.diff) #vetor de numeros inteiros
  MATRIX <- matrix(1:(length(ts.diff)*n), ncol = n) 
  for (i in 1:(length(ts.diff))) {
    temp = ""
    for (j in 1:n) {
